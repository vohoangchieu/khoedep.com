$(function() {
    ...
    try {/*
     tinymce.init({
     external_image_list_url : "http://khoan.com/ajax/am/get-list-image.js",
     selector: "textarea",
     theme: "modern",
     plugins: [
...
//        console.log(e);
    ...
    $("#update-product").click(handleUpdate);
    $("#delete-product").click(handleDelete);
})
function handleDelete() {
    if (confirm("Bạn muốn xóa sản phẩm này?")) {
        var id = $("#id").val();
        $.ajax({
            url: "/ajax/am/delete-product",
            type: 'POST',
            dataType: 'json',
            data: {
                id: id
...
//            console.log(data);
            if (data.code == 0) {
                var link = '<a style="color:#fff" href="/' + $("#id").val() + '" target="_blank">' + $("#name").val() + "</a>";
                $.ambiance({
                    type: "success",
...
function validateInput() {
    if ($("#name").val().trim() == "") {
        $("#error").html("Nhap ten san pham");
        return false;
    }
    if ($("#price").val().trim() == '') {
        $("#error").html("Nhap gia san pham");
        return false;
    }
    if ($("#warranty").val().trim() == '') {
        $("#error").html("Nhap thoi gian bao hanh san pham");
        return false;
    }
...
$(function() {
    'use strict';
    var url = '/ajax/am/update-product-image-handler/';
